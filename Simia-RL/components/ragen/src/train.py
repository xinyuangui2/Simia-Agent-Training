...
def add_dependency_and_validate_config(config):
    ...
    if os.path.isdir(model_name):
        with open(os.path.join(model_name, "config.json"), "r") as f:
            ...
            model_name = model_config.get("_name_or_path", model_name)

    if config.trainer.project_name is None:
        config.trainer.project_name = os.environ.get("MLFLOW_EXPERIMENT_NAME", None)

    if config.system.CUDA_VISIBLE_DEVICES is None:
        default_cuda_visible_devices = "-1"
        if "NVIDIA_VISIBLE_DEVICES" in os.environ:
            num_gpus = len(os.environ["NVIDIA_VISIBLE_DEVICES"].split(","))
            default_cuda_visible_devices = ",".join(list(map(str, range(num_gpus))))
        config.system.CUDA_VISIBLE_DEVICES = os.environ.get("CUDA_VISIBLE_DEVICES", default_cuda_visible_devices)
        print(f"system.CUDA_VISIBLE_DEVICES not set, using {config.system.CUDA_VISIBLE_DEVICES} as default")

    if config.trainer.n_gpus_per_node is None:
        config.trainer.n_gpus_per_node = len(str(config.system.CUDA_VISIBLE_DEVICES).split(','))
        print(f"trainer.n_gpus_per_node not set, using {config.trainer.n_gpus_per_node} as default")

    if config.actor_rollout_ref.rollout.tensor_model_parallel_size is None:
        config.actor_rollout_ref.rollout.tensor_model_parallel_size = config.trainer.n_gpus_per_node
        print(f"actor_rollout_ref.rollout.tensor_model_parallel_size not set, using {config.actor_rollout_ref.rollout.tensor_model_parallel_size} as default")
    ...
    assert "qwen" in model_name.lower() or (not config.enable_response_mask), \
...
@hydra.main(version_base=None, config_path="default_config", config_name="user_config")
def main(config):
