...
class EnvStateManager:
    ...
    """
    def __init__(self, config, mode: str = "train", tokenizer=None):
    ...
    def _init_env_instances(self, config):
        ...
        for tag, n_group in zip(config.env_configs.tags, config.env_configs.n_groups):
            for env_id in range(done_groups * self.group_size, (done_groups + n_group) * self.group_size):
                ...
                # For office env and simulated env, we also need to pass env_id to the config
                if env_class in ['officebench', 'simulated', 'simulated_general']:
                    if cfg_template.env_config is None:
                        env_config = REGISTERED_ENV_CONFIGS[env_class](env_id=str(env_id))
                    else:
                        cfg_template.env_config['env_id'] = str(env_id)
                ...
                # Pass tokenizer and max_model_len to simulated_general env
                env_kwargs = {'group_size': self.group_size}
                if env_class == 'simulated_general' and self.tokenizer is not None:
                    env_kwargs['tokenizer'] = self.tokenizer
                    max_model_len = self.sys_config.actor_rollout_ref.rollout.get('max_model_len', 9000)
                    env_kwargs['max_model_len'] = max_model_len
                
                env_obj = REGISTERED_ENVS[env_class](env_config, **env_kwargs)
    ...
    def reset(self, seed: Optional[int] = None):
        ...
        for cache, env in zip(rollout_cache, envs):
            ...
            # For simulated_general env, include system_prompt in cache
            system_prompt = getattr(env['env'], 'system_prompt', None)
            if system_prompt:
                cache['system_prompt'] = system_prompt
    ...
    def _process_single_env(self, env_input: Dict):
        """Process a single environment input - extracted for parallel processing"""
        ...
        if len(valid_actions) != len(env_input['actions']) or not valid_actions:
            env.observation = "Wrong format for the response. Expected <think>...</think>\n<answer>[JSON payload]</answer>"
    ...
    def step(self, all_env_inputs: List[Dict]):
        ...
        with ThreadPoolExecutor(max_workers=min(len(all_env_inputs), 16)) as executor:
            future_to_env = {executor.submit(self._process_single_env, env_input): env_input for env_input in all_env_inputs}

            for future in as_completed(future_to_env):
                env_id, turn_done = future.result()
        ...
        for entry, cache in zip(envs, rollout_cache):
            ...
            for turn in cache['history']:
                for k, v in turn.get('info', {}).items():
                    if k == 'success':
                        continue
                    # Skip non-numeric fields like termination_reason
                    if isinstance(v, str):
                        continue
