...
class LLMAgentProxy:
	...
	def __init__(self, config, actor_rollout_wg, tokenizer):
		...
		self.train_ctx_manager = ContextManager(config, self.train_es_manager.envs, tokenizer, mode="train")
		self.val_es_manager = EnvStateManager(config, mode="val")
		self.val_ctx_manager = ContextManager(config, self.val_es_manager.envs, tokenizer, mode="val")
