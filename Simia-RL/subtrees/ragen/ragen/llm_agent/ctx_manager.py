...
class ContextManager:
    ...
    def _parse_response(self, response: str) -> List:
        # For GeneralSimulated: return the whole response as action (no special token parsing)
        llm_response = response
        actions = [response.strip()] if response.strip() else []
    ...
    def get_lm_inputs(self, env_outputs: List[Dict], prepare_for_update: bool) -> DataProto:
        ...
        for env_output in env_outputs:
            ...
            # Use system_prompt from env_output if available (for simulated_general), otherwise use default
            system_content = env_output.get('system_prompt', "You're a helpful assistant.")
            user_initial_content = self.prefix_lookup[env_output["env_id"]]

            messages = [
                {"role": "system", "content": system_content}
            ]

            # Only add initial user message if there's env_instruction
            if user_initial_content:
                messages.append({"role": "user", "content": user_initial_content})

            # Build conversation history from env_output
            for idx, content in enumerate(env_output["history"]):
                if "state" in content:
                    # State is the GPT-generated message (human or tool response)
                    messages.append({"role": "user", "content": content['state']})
                if "llm_response" in content:
                    # LLM response is added as assistant message directly
            ...
            existing_env_ids.append(env_output["env_id"])

        # Tokenize with truncation based on max_model_len
        response_length = self.config.actor_rollout_ref.rollout.response_length
        max_model_len = self.config.actor_rollout_ref.rollout.max_model_len
        # Truncate to max_model_len - response_length - 500, leaving extra space for new generation
        max_prompt_len = max_model_len - 500

        inputs = self.tokenizer(llm_input_texts, return_tensors="pt", padding=True, padding_side="left", truncation=True, max_length=max_prompt_len)
    ...
    def get_env_inputs(self, lm_outputs: DataProto) -> List[Dict]:
        ...
        # For GeneralSimulated, don't add any special tokens
        # For other envs, add <think> or <answer> tags
        env_tag = self.config.es_manager['train']['env_configs']['tags'][0]
        if env_tag != 'GeneralSimulated':
