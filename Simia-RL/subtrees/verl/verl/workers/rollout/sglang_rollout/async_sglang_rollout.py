...
class AsyncSGLangRollout(BaseRollout):
    ...
    def generate_sequences_with_tools(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        for req in sorted_output_req_list:
            ...
            # If input_ids exceed max_model_len, truncate instead of throwing error
            if len(req.input_ids) > self.config.max_model_len:
                print(f"Warning: Request {req.request_id} has input_ids length {len(req.input_ids)} greater than max_model_len {self.config.max_model_len}. Truncating prompt from left.")
                print(f"Decoded input_ids before truncation: {self.tokenizer.decode(req.input_ids)}")
                # Call truncate_output_ids method to perform truncation
                req.truncate_output_ids(self.tokenizer)
                print(f"After truncation, input_ids length: {len(req.input_ids)}")
